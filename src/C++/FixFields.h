...
namespace FIX
{
  ...
  DEFINE_STRING(BenchmarkSecurityDesc);
  DEFINE_PRICE(LegDV01);
  DEFINE_INT(SpreadType);
  DEFINE_PRICE(LegNotionalNeutral);
  DEFINE_INT(PriceScalingFactor);
  DEFINE_INT(VolumeScalingFactor);
  DEFINE_STRING(SetupMonitorRequestID);
  DEFINE_NUMINGROUP(NoMonitorIDs);
  DEFINE_STRING(MonitorID);
  DEFINE_BOOLEAN(ClearMonitorIndicator);
  DEFINE_CHAR(MonitorSortOption);
  DEFINE_INT(MonitorNumber);
  DEFINE_INT(MonitorPageNumber);
  DEFINE_NUMINGROUP(NoMonitorRows);
  DEFINE_INT(RowNumber);
  DEFINE_INT(RowLoadAction);
  DEFINE_INT(RowForegroundColor);
  DEFINE_STRING(SecurityIDSourceOverride);
  DEFINE_BOOLEAN(SuppressPricesIndicator);
  DEFINE_INT(SetupMonitorRequestStatus);
}
#endif //FIX_FIELDS_H
