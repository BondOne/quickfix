...
namespace FIX
{
  ...
  const char MsgType_QuoteAcknowledgement[] = "CW";
  const char MsgType_PartyDetailsListRequest[] = "CF";
  const char MsgType_PartyDetailsListReport[] = "CG";
  const char MsgType_SetupMonitorRequest[] = "USMQ";
  const char MsgType_SetupMonitorRequestAck[] = "USMQK";
  ...
  const int SpreadType_BENCHMARK = 0;
  const int SpreadType_BREAKEVEN = 1;
  const int SpreadType_EFP_RATE = 2;
  const char MonitorSortOption_ABSOLUTE = 'A';
  const char MonitorSortOption_MATURITY = 'M';
  const int RowLoadAction_ADD_ROW = 0;
  const int RowLoadAction_MODIFY_ROW = 1;
  const int RowLoadAction_DELETE_ROW = 2;
  const int RowForegroundColor_AMBER = 2;
  const int RowForegroundColor_WHITE = 4;
  const char SecurityIDSourceOverride_CUSIP[] = "1";
  const char SecurityIDSourceOverride_SEDOL[] = "2";
  const char SecurityIDSourceOverride_ISIN_NUMBER[] = "4";
  const char SecurityIDSourceOverride_ISO_CURRENCY_CODE[] = "6";
  const char SecurityIDSourceOverride_EXCHANGE_SYMBOL[] = "8";
  const char SecurityIDSourceOverride_BLOOMBERG_SYMBOL[] = "A";
  const char SecurityIDSourceOverride_WERTPAPIER[] = "B";
  const char SecurityIDSourceOverride_VALOREN[] = "D";
  const char SecurityIDSourceOverride_MARKETPLACE_ASSIGNED_IDENTIFIER[] = "M";
  const int SetupMonitorRequestStatus_ACCEPTED = 0;
  const int SetupMonitorRequestStatus_REJECTED = 1;
  }
  #endif //FIX_VALUES_H
